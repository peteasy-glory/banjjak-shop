...
var common = {
    ...
    init: function () {		
		...
		//버튼 프레스 이벤트
		$(document).on('touchstart touchend mousedown mouseup mouseleave' , 'a , button , .form-toggle-box , .form-toggle-options , .btn-text-button , .btn-tab-item , .form-switch-toggle , .ui-slider-handle , .btn-reserve-calendar-sort , .btn-login-item , .btn-accordion-menu , .btn-page-cart , .btn-product-option-minus , .btn-product-option-plus , .form-radiobox' , function(e){
			
			if(e.type == 'touchstart' || e.type == 'mousedown'){
				//console.log('start');
				if(!$(this).hasClass('press')) $(this).addClass('press');
			}else if(e.type =='touchend' || e.type == 'mouseup' || e.type == 'mouseleave'){
				//console.log('end');
				if($(this).hasClass('press')) $(this).removeClass('press');
			};
