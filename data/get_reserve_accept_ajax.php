...
if(count($arr)>0){
    foreach($arr as $rs){
//        print_r($rs);
        $que = "SELECT a.*, b.file_path FROM tb_mypet a LEFT JOIN tb_mypet_beauty_photo b ON a.pet_seq = b.pet_seq WHERE a.pet_seq = '{$rs['pet_seq']}' ORDER BY upload_dt ASC LIMIT 1";
        //echo $que."<br>";
        $res = sql_query($que);
        $row = sql_fetch($res);
        if($row['pet_seq']){
            //echo $row['photo'];
            $html .= '<a href="./reserve_accept_1_input?cellPhone='.$rs['cellphone'].'&worker='.$worker.'&workTime='.$workTime.'&workDate='.$workDate.'&no='.$row['pet_seq'].'&searchTxt='.$search.'" class="customer-card-item pet">';
            $html .= '    <div class="item-info-wrap">';
            $html .= '        <div class="item-thumb">';
            if(empty($row['photo'])) {
                $html .= '            <!-- 이미지 없을때 -->';
                $html .= '            <div class="user-thumb large"></div>';
            } else {
                $html .= '            <!-- 이미지 있을때 -->';
