...
class Pagination {
	...
	var $num_links			=  0; // 좌우측값 대칭 페이징때 사용
...
//		}
		...
		if ($this->first_link !== FALSE AND $this->cur_page > ($this->num_links + 1)) {
			...
			$output .= $this->first_tag_open.'<a class="page-link" href="'.$first_url.'"'.$this->n_rep($this->onclick, $i).'>'.$this->first_link.'</a>'.$this->first_tag_close;
		}
		...
		if ($this->prev_link !== FALSE AND $this->cur_page != 1) {
			...
			if ($i == 0 && $this->first_url != '') {
				$output .= $this->prev_tag_open.'<a  class="page-link" href="'.$this->first_url.'">'.$this->prev_link.'</a>'.$this->prev_tag_close;
			}
			else {
				$i = ($i == 0) ? '' : $this->prefix.$i.$this->suffix;
				$output .= $this->prev_tag_open.'<a  class="page-link" href="'.$this->base_url.$i.'"'.$this->n_rep($this->onclick, $i).'>'.$this->prev_link.'</a>'.$this->prev_tag_close;
			}
		}
			...
			for ($loop = $start -1; $loop <= $end; $loop++) {
				...
				if ($i >= $base_page) {
					if ($this->cur_page == $loop) {
						...
						if ($n == '' && $this->first_url != '') {
							$output .= $this->num_tag_open.'<a class="page-link" href="'.$this->first_url.'">'.$loop.'</a>'.$this->num_tag_close;
						}
						else {
							$n = ($n == '') ? '' : $this->prefix.$n.$this->suffix;

							$output .= $this->num_tag_open.'<a class="page-link" href="'.$this->base_url.$n.'"'.$this->n_rep($this->onclick, $n).'>'.$loop.'</a>'.$this->num_tag_close;
						}
					}
				}
			}
		...
		if ($this->next_link !== FALSE AND $this->cur_page < $this->num_pages) {
			...
			$output .= $this->next_tag_open.'<a class="page-link" '.$this->anchor_class.'href="'.$this->base_url.$this->prefix.$i.$this->suffix.'"'.$this->n_rep($this->onclick, $i).'>'.$this->next_link.'</a>'.$this->next_tag_close;
		}
		...
		if ($this->last_link !== FALSE AND ($this->cur_page + $this->num_links) < $this->num_pages) {
			...
			$output .= $this->last_tag_open.'<a class="page-link" '.$this->anchor_class.'href="'.$this->base_url.$this->prefix.$i.$this->suffix.'"'.$this->n_rep($this->onclick, $i).'>'.$this->last_link.'</a>'.$this->last_tag_close;
		}
